...
const useAllotmentListManager = () => {
    ...
    const [filters, setFilters] = useState({
        ...
        hs_code: '',
        exporter: null,
        include_assigned: false, // default excludes BOE-assigned on server
    });
    ...
    const buildParams = useCallback((pageOverride = 1) => {
        const params = {
            page: pageOverride,
        ...
        if (filters.include_assigned) {
            // show only with BOE assigned (has_boe=true)
            params.has_boe = 'true';
        }
        // If include_assigned is false, omit has_boe to use the server default (exclude BOE)
    ...
    const fetchData = useCallback(async (pageOverride = 1, append = false) => {
        setLoading(true);
        try {
            const res = await axios.get('/api/allotments/', {params: buildParams(pageOverride)});
            const data = res.data;
            if (append) {
    ...
    const updateSingleEntry = useCallback(async (id) => {
        try {
            const res = await axios.get(`/api/allotments/${id}/`);
