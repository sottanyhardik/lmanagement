...
import AllotmentLineItemTable from './AllotmentLineItemTable';

const AllotmentForm = ({entry, isNew = false, onClose, onSaved}) => {
    ...
    const save = async () => {
        ...
        try {
            const payload = {
                type: 'AT', // default server-side too; kept explicit
                ...
                <div className="alert alert-danger py-2 small">
                    ...
                    <ul className="mb-0 ps-3">
                        {Object.entries(errors).map(([field, msg], i) => (
                            <li key={i}>
                                <span
                                    className="text-capitalize">{field.replace(/_/g, ' ')}</span>: {Array.isArray(msg) ? msg.join(', ') : msg}
                            </li>
                        ))}
                    </ul>
                </div>
