...
import {getChoiceLoader} from '../../hooks/getChoiceLoader.js'; // adjust path if needed
...
export default function ChoiceSelect({
                                         ...
                                         isClearable = true,
                                         isDisabled = false,
                                         returnValues = false,              // if true, onChange returns only value(s)
                                         defaultOptionsCount = 100,         // how many options to show before typing
                                         instanceId,
                                         ...props
                                     }) {
    const {choices, loading} = useLicenseChoices(); // { [key]: [{label, value}, ...] }
    const group = useMemo(() => choices?.[choiceKey] || [], [choices, choiceKey]);

    // Fast value->option map for resolving the controlled value
    ...
    // Client-side async loader backed by the full choices map
    const loadOptions = useMemo(
        () => getChoiceLoader(
            {[choiceKey]: group}, // just pass the slice we need
            choiceKey,
    ...
    if (loading && !group.length) {
        return <div style={{fontSize: 12, opacity: 0.7}}>Loading choices for {String(choiceKey)}…</div>;
    }
